												...
												<tr>
													...
													<td class="px-4 py-3" style="text-align: center">
														<div class="flex items-center space-x-4 text-sm">
															...
															</a> </a> <a href="BookAppointmentDoctor?id=<%=rs.getString(1)%>"
																class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
																aria-label="reset"> <svg
																	xmlns="http://www.w3.org/2000/svg" width="16"
...
<script>
		$('body').on('keyup', '#search', function() {
			...
			var url = 'DoctorSearch?id=' + searchQuest;
			$.ajax({
				url : url,
				type : 'get',
				dataType : 'json',
				success : function(response) {
						...
						if (Array.isArray(response) && response.length == 0) {
			            	...
			            	for(val in response) {
			            		var url1 = 'DoctorInfos?id=' + response[val].id;
			            		var url2 = 'editDoctor?id=' + response[val].id;
			            		var url3 = 'resetPasswordDoctor?id=' + response[val].id;
			            		var url4 = 'BookAppointmentDoctor?id=' + response[val].id;
			            		var url5 = 'DoctorSoftDelete?id=' + response[val].id;
			            		
								var workinDays = "";
								for(i in response[val].workingDays) {
									workinDays += response[val].workingDays[i] + "-"
								}
								console.log("workinDays " + workinDays);
			            		searchResult = '<tr>' +
